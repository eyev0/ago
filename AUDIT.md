...
| # | Проблема | Серьёзность | Новая? | Статус |
|---|---------|-------------|--------|--------|
...
| ~~M3~~ | ~~Agent-log-entry: неполный список статусов~~ | ~~Средняя~~ | — | **ИСПРАВЛЕНО** |
...
| ~~L1~~ | ~~task-lifecycle не ссылается на naming~~ | ~~Низкая~~ | — | **ИСПРАВЛЕНО** |
| ~~L2~~ | ~~Роль DOC не ссылается на documentation.md~~ | ~~Низкая~~ | — | **ИСПРАВЛЕНО** |
| ~~L3~~ | ~~target_date без пояснений~~ | ~~Низкая~~ | — | **ИСПРАВЛЕНО** |
...
**Все проблемы исправлены.** ✓ (27 AUDIT + 17 patch queue, consolidated from TODO.md)

---

## Patch Queue (consolidated from TODO.md)

All 17 patches resolved. Cross-references to AUDIT issues shown in parentheses.

- [x] `P01` Add/remove TODO.md dependency for workflow-developer role
- [x] `P02` Add missing timeline template (→ M4)
- [x] `P03` Normalize all paths to `.workflow/...` (→ C1, H7, H8)
- [x] `P04` Lowercase log directory casing (→ H2)
- [x] `P05` Short task ID (`T001`) as canonical identifier
- [x] `P06` Reconcile blocked → planned | in_progress transitions
- [x] `P07` Clarify documentation ownership (owner edits, others propose)
- [x] `P08` Distinguish DR proposals vs formal DR creation by CONS
- [x] `P09` Narrow frontmatter rule to entity docs, exempt logs (→ M2)
- [x] `P10` Standardize `ago:` syntax without leading slash (→ M5)
- [x] `P11` Align readiness messaging with implementation status
- [x] `P12` Narrow Codex claims to "planned" (→ M1)
- [x] `P13` DOC scope limited to `.workflow/` only
- [x] `P14` MASTER logs own transitions in master log only
- [x] `P15` `ago:clarify` creates task.md after APPROVE step
- [x] `P16` Remove hardcoded path from config template
- [x] `P17` Wikilinks use full slug format
